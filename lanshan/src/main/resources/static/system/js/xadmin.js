;!function (win) {
  ...
  Xadmin.prototype.set_cate_data = function(data) {
	 ...
	 * 公共的Ajax提交方法
	 */
	Xadmin.prototype.ajaxPost = function (pageurl,listurl,mydata,msg,callback){
		...
		jQuery.ajax({
			...
			success:function(ret){
				...
				if(ret.status=="success"){
					...
					layer.msg('sorry,操作失败了 ...', {icon: 2,time:2000});
					layer.closeAll('loading')
				}
			},
			error:function(){
	 ...
	 * 公共的Ajax提交方法
	 */
	Xadmin.prototype.ajaxPostAlt = function (pageurl,listurl,mydata,callback){
		var index = layer.load(null, {shade: [1, '#393D49'] });
		if(pageurl==null||pageurl==''){
			layer.alert('提交地址不能为空！', {icon: 5});
			layer.closeAll('loading')
			return false;
