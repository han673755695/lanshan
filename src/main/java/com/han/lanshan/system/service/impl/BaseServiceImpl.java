...
import com.han.lanshan.system.utils.OpenOfficeKit;
import com.han.lanshan.system.utils.SpringContextUtil;
 ...
 * 
  * 描述：   
  * 创建人：HYD
  * 创建时间：2019年9月8日 上午11:55:55   
  * 修改人：HYD
  * 修改时间：2019年9月8日 上午11:55:55
  * 修改备注：   
 * @version
 */
public abstract class BaseServiceImpl implements IBaseService {
	...
	@Override
	public <T> File findDataExportExcel(String ftlurl, Page page, Class<T> clazz, Object queryBean) throws Exception {
		Map map = new HashMap();
		return findDataExportExcel(ftlurl, page, clazz, queryBean, map);
	}
	
	@Override
	public <T> File findDataExportExcel(String ftlurl, Page page, Class<T> clazz,
			Object queryBean, Map map) throws Exception {
		
		return findDataExportFile(ftlurl, page, clazz, queryBean, map, ".xls");
	}
	
	
	public <T> File findDataExportFile(String ftlurl, Page page, Class<T> clazz, Object queryBean,
		...
		// Map map = new HashMap();
		ReturnData returnDatas = new ReturnData();
		...
		List datas = findListDataByParams(queryBean, clazz, page);
		returnDatas.setData(datas);
		returnDatas.setPage(page);
		returnDatas.setQueryParams(queryBean);
		...
		if(ftlurl.contains("Xml"))
			first = false;
		boolean end = false;
		int pageCount = page.getCurrentPage();
		if (pageCount < 2) {
			pageCount = 1;
			end = true;
		}
		...
		for (int i = 2; i <= pageCount; i++) {
			...
			page.setCurrentPage(1);
			datas = findListDataByParams(queryBean, clazz, page);
			returnDatas.setData(datas);
			map.put(GlobalStatic.returnDatas, returnDatas);
			createExceFile(template, ffile, excelFile, first, end, map);
		}
		...
		try {
			File excelnew = new File(
					GlobalStatic.tempRootpath + "/" + fileName + "/" + UUIDUtils.getUUID() + GlobalStatic.excelext);
