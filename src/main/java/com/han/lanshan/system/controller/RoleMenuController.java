...
public class RoleMenuController extends BaseController {
	...
	public String list(HttpServletRequest request, Model model, RoleMenu roleMenu) {
		ReturnData returnObject = listjson(request, model, roleMenu);
		model.addAttribute(GlobalStatic.returnDatas, returnObject);
		return "/system/roleMenu/roleMenuList";
	}
	...
	@ResponseBody
	public ReturnData listjson(HttpServletRequest request, Model model, RoleMenu roleMenu) {
		ReturnData returnData = ReturnData.getSuccess();
		
		try {
			Page page = Page.getPage(request);
			List<RoleMenu> datas = roleMenuService.findRoleMenuList(roleMenu, page);
			int totalCount = roleMenuService.findRoleMenuCount(roleMenu, page);
			page.setTotalCount(totalCount);
			returnData.setQueryParams(roleMenu);
	...
	public void listexport(HttpServletRequest request, HttpServletResponse response, Model model, RoleMenu roleMenu) {
		try {
			Page page = Page.getPage(request);
			File file = roleMenuService.findDataExportExcel("/system/roleMenu/roleMenuList", page, RoleMenu.class, roleMenu);
	...
	@ResponseBody      
	public ReturnData saveorupdate(Model model, RoleMenu roleMenu, HttpServletRequest request, HttpServletResponse response) {
		ReturnData returnObject = ReturnData.getSuccess();
		try {
			...
			roleMenuService.saveorupdateRoleMenu(roleMenu, true);
		...
		try {
			...
			if(StringUtils.isNotBlank(ids)){
				String[] idsArr = ids.split(",");
			 	roleMenuService.deleteRoleMenu(Arrays.asList(idsArr));
			} else {
				returnObject.setStatus(ReturnData.ERROR);
